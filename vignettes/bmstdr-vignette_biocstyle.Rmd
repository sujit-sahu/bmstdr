...
R packages for model fitting. The exact form of the models in each case is documented in Chapter 6 of the book @Sahubook. 
 ...
 Calculation of model choice statistics is triggered by the option `mchoice=TRUE`. In this case the DIC, WAIC and PMCC
...
```{r}
M1 <- Bspatial(formula=yo3~xmaxtemp+xwdsp+xrh, data=nyspatial, mchoice=TRUE)
```
...
M2 <- Bspatial(model="spat", formula=yo3~xmaxtemp+xwdsp+xrh, data=nyspatial, 
      coordtype="utm", coords=4:5, phi=0.4, mchoice=TRUE)
```
...
M3 <- Bspatial(package="spBayes", formula=yo3~xmaxtemp+xwdsp+xrh, data=nyspatial, 
               coordtype="utm", coords=4:5, prior.phi=c(0.005, 2), mchoice=TRUE)
...
The `r CRANpkg("bmstdr")` command for performing validation needs an additional argument `validrows` which are the row numbers of the supplied data frame which should be used for validation. Thus the commands for validating at the
...
```{r valplot1, echo=TRUE, eval=TRUE, message=FALSE, results='hide', fig.cap="Prediction against observation plot with the prediction intervals included. The `in/out' symbol in the plot indicates whether or not a prediction interval incudes the 45 degree line."}
...
several plots depending on the contents of the model fitted object. For example, this always draws a residuals against fitted values plot. The `plot` command for objects of class `r CRANpkg("bmstdr")` can take  additional arguments such as `segments = F` 
...
Now we use the `spTimer` package to fit and validate the model. 
...
M31 <- Bsptime(package="spTimer",formula=f2, data=nysptime, 
               ...
               mchoice=FALSE, scale.transform = "NONE", N=5000)
```
...
The  function `fig11.13.plot` is used to generate this plot.
...
```{r valid3sites, echo=TRUE, eval=TRUE, fig.height=9, fig.cap="Time series of observed and predicted values at site number 10."}
...
`interp` in the `akima` library. Also we discard the interpolations outside the state of New York by using the function `fnc.delete.map.XYZ`.   
...
ggsn::scalebar(data =interp1, dist = 100, location = "bottomleft", transform=TRUE, dist_unit = "km", st.dist = .05, st.size = 5, height = .06, st.bottom=TRUE, model="WGS84") +
ggsn::north(data=interp1, location="topleft", symbol=12) +
labs(x="Longitude", y = "Latitude", size=2.5) 
```

The predicted map is plotted below. 
```{r, echo=FALSE, eval=TRUE, fig.height=6, fig.width=8, fig.cap="Predicted map of average ozone air pollution in New York"}
P
```
...
M4.c <- Bsptime(package="stan",formula=f2, data=nysptime, 
        coordtype="utm", coords=4:5, scale.transform = "SQRT", 
        N=1500, burn.in=500, mchoice=TRUE, verbose = F)
```
...
M5 <- Bsptime(package="spTimer", model="AR", formula=f2, data=nysptime, 
                coordtype="utm", coords=4:5, scale.transform = "SQRT", 
                mchoice=TRUE,  N=5000, validrows = vrows)
...
```{r atllocationsdeep, echo=TRUE, eval=TRUE, fig.cap="Locations of moving Argo floats in the deep ocean in 2003."}
...
deep ocean is `deep`, we use the 
 command 
```{r deepmodfitting, echo=TRUE, eval=TRUE, results='hide', message=FALSE}
Natl <- 25
Nburn <- 5
...
patldeep <- ggplot() +  
  ...
  geom_contour(data=na.omit(interp1), aes(x = long, y = lat,z = Temp)) + 
  geom_polygon(data=atlmap, aes(x=long, y=lat, group=group),
               color="black", size = 0.6, fill=NA) + 
    geom_point(data =deep, aes(x=lon, y=lat, colour=month), size=1) +
...
The `engdeaths` data set contains 49,292 weekly recorded deaths during this period of 20 weeks. The boxplot of the weekly death rates  shows the first peak during weeks 15 and 16
...
```{r ptime, echo=TRUE, eval=TRUE, fig.cap="Weekly Covid-19 death rate per 100,000"}
...
M3.inla.bym <- Bcartime(formula=f3, data=engtotals, scol ="spaceid",  
                        ...
                        W=Weng,  package="inla", N=N, burn.in=burn.in, thin=thin) 
...
```
 ...
 argument `interaction=FALSE` may be supplied to suppress the interaction term.
...
M2stinla.0  <- Bcartime(data=engdeaths, formula=f2inla, W=Weng, scol =scol, tcol=tcol,  
                        offsetcol="logEdeaths",  model=model,  link="log", family="poisson", package="inla", validrow=vs, N=N, burn.in=burn.in, thin=thin) 
...
ar2valid <- b$pwithseg
```
```{r obsvpredplot, echo=FALSE, eval=TRUE, fig.height=7, fig.width=6, fig.cap="Predictions with 95% limits against observations for two models: AR (2) on the top panel and INLA on the bottom panel."}
library(ggpubr)
