...
`INLA` based models use  a discretized Gaussian Markov random field with penalized complexity prior distribution [@Fuglstad2018]. 

For areal unit data modeling the `bmstdr` function `Bcartime` provides opportunities for selected model fitting using `CARBayes` [@LeeCARBayes2021], and  `CARBayesST` [@CarBayesST] 
 `CARBayesST`[@CarBayesST] and also  `INLA` as illustrated by  @blangiardoandcameletti. Here also a base line Bayesian generalized linear model for independent data, fitted using `CARBayes`,  is included for model comparison purposes. The `INLA` based models can fit the celebrated @BYM model and the Leroux model [@Leroux2000]. 
...
for `M4` takes about 20 minutes on a fast personal computer. These run produces the values of various Bayesian model choice criteria shown in Table \@ref(tab:tab). 
```{r tab, echo=FALSE, eval=TRUE}
   ...
   set_caption('Model choice criteria for various models fitted to the nyspatial data set.')
...
```{r mvalidnyspatial, echo=FALSE, eval=TRUE}
   table2[-4, ] %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(3)     %>%
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red")%>% 
   set_caption('Model validation statistics for the five models fitted to the nyspatial data set.')
```

To illustrate $K$-fold cross-validation, the 28 observations  in the `nyspatial` data set are randomly assigned to $K=4$ groups of equal size and the model fitting command is issued four times. For brevity only one of these four commands is shown below. The command draws a validation plot of the predictions 
against observation. 
```{r, valplot1, echo=T, eval=TRUE, fig.width=6, fig.height=4, message=FALSE, results='hide', fig.cap="Prediction against observation plot with the prediction intervals included. The `in/out' symbol in the plot indicates whether or not a prediction interval incudes the 45 degree line."}
set.seed(44)
x <- runif(n=28)
u <- order(x)
s1 <- u[1:7]
...
Table   \@ref(tab:m2-4-fold-validnyspatial) presents the 4-fold cross-validation statistics for `M2` only.  It shows a wide variability in performance
with a low coverage of 57.14% for Fold 3. 
```{r m2-4-fold-validnyspatial, echo=FALSE, eval=TRUE}
   table3 %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(3)     %>%
   map_text_color(by_cols("darkred", "blue", "darkgreen", "purple")) %>%
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>%
   set_caption('4-fold cross-validation statistics for model M2 fitted to the nyspatial data set.')
...
To explore the model fitted object issue the command `names(M2)`; to extract and plot 
the residuals  use the command `a <- residuals(M2)`. 
...
For example:
```{r, echo=T, eval=F,message=FALSE, results='hide' }
...
```{r vsites3, echo=FALSE, eval=TRUE, fig.cap="25 fitted sites and 3 validation sites (numbered) in  New York", fig.width=5, fig.height=4}
...
label <- tibble(
   long = -76.1,
   lat = 41.5,
   label = "27 fitted (circles) & validation site 10"
)
...
M31 <- Bsptime(package="spTimer",formula=f2, data=nysptime, 
               ...
               validrows=vrows, model="GP", report=2, 
...
```{r valid3sites, echo=T, eval=T, fig.width =7,  fig.height=4, fig.cap="Time series of observed and predicted values at site number 10."}
p1 <- fig11.13.plot(yobs[1, ], y.valids.low[1, ], y.valids.med[1, ], 
                    y.valids.up[1, ], misst=validt)
p1 <- p1 + ggtitle("Validation for Site 10")
p1
```
...
```{r mchoice-m1-m4, echo=FALSE, eval=TRUE}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Model choice  criteria for the four spatio-temporal models M1 to M4.')
```
...
```{r mcvm5m6, echo=FALSE, eval=TRUE}
   table5 %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(2)     %>%
   map_text_color(by_cols("darkred", "blue", "darkgreen")) %>%
...
```{r mparm-m5-m6, echo=FALSE, eval=TRUE}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Parameter estimates for the two AR models M5 (first four columns) and M6 (last four columns).')
```


## Spatio-temporal dynamic models using the spTDyn and spBayes packages 
...
```
The dynamic model parameters can be  extracted as has been illustrated in the @spBayes package. 
...
```{r modv-m1-m9, echo=FALSE, eval=TRUE}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Model choice and validation statistics for the eight models.')
```
...
```{r atllocationsdeep, echo=T, eval=T, fig.width =6,  fig.height=4, fig.cap="Locations of moving Argo floats in the deep ocean in 2003."}
...
```
to fit model \@ref(eq:marginalytocean) implemented by code written in   `rstan`. Like the `Bsptime` function this model fitting function also renders a  `bmstdr` object which can be further investigated. 

The model output is not shown here. But we obtain an  annual prediction map by averaging over the model equation  \@ref(eq:marginalytocean) at each data location. The posterior samples contained in `M2` are used to sample the annual predictions and then those samples are linearly interpolated using the `akima` library to obtain the  prediction map in Figure \@ref(fig:oceanplot) and the uncertainty map in \@ref(fig:oceanplotsd). 
```{r processdeep, echo=FALSE, eval=TRUE, message=FALSE, include=FALSE}
...
patldeep <- ggplot() +  
  ...
  geom_polygon(data=atlmap, aes(x=long, y=lat, group=group),
               color="black", size = 0.6, fill=NA) + 
    geom_point(data =deep, aes(x=lon, y=lat, colour=month), size=1) +
  labs( title= "Predicted annual temperature in deep ocean in 2003", x="Longitude", y = "Latitude") +
  ggsn::scalebar(data =atlmap, dist = 1000, location = "bottomleft", transform=T, dist_unit = "km",
                 st.dist = .05, st.size =5, height = .05, st.bottom=T, model="WGS84") +
  ggsn::north(data=atlmap, location="topright", symbol=12) 
...
```{r oceanplot, message=FALSE, results='hide',  echo=FALSE, eval=TRUE, fig.width =6,  fig.height=4, fig.cap="Predicted annual temperature  map of the  deep ocean in 2003"}
patldeep
```
...
```{r staticbinomial, echo=T, eval=T, message=FALSE, results='hide'}
...
```{r logistic, echo=FALSE, eval=TRUE}
 a %>%
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Comparison of logistic regession models for static areal data')
```
...
```{r staticpoisson, echo=T, eval=T, message=FALSE, results='hide'}
...
```{r, echo=FALSE, eval=T, message=FALSE, results='hide'}
...
```{r poisson, echo=FALSE, eval=TRUE}
a %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(2)     %>%
   map_text_color(by_cols("darkred", "blue", "darkgreen")) %>%
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>%
   set_caption('Comparison of disease mapping models for Covid-19 mortality')
```
...
```{r normalstaticfit, echo=T, eval=T, message=FALSE, results='hide'}
...
```{r staticnormal, echo=FALSE, eval=TRUE}
   a %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(2)     %>%
   map_text_color(by_cols("darkred", "blue", "darkgreen")) %>%
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Comparison of Gaussian models for NO2 data')
```
...
```
 ...
 ```{r m1st, echo=FALSE, eval=TRUE}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Model choice criteria values for various spatio-temporal binomial model.')
```
...
```{r m2st, echo=FALSE, eval=TRUE}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red")%>% 
   set_caption('Model choice criteria values for the spatio-temporal Poisson model.')
```
...
```{r m2st-valid, echo=FALSE, eval=TRUE}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red")%>% 
   set_caption('Model validation statistics for the Anova and AR model.')
```
...
ar2valid <- b$pwithseg
```
```{r obsvpredplot, echo=FALSE, eval=TRUE, fig.height=7, fig.width=6, fig.cap="Predictions with 95% limits against observations for two models: AR (2) on the left panel and INLA on the right panel."}
library(ggpubr)
...
```{r m3st-no2, echo=FALSE, eval=TRUE, caption="Model choice criteria values for the spatio-temporal Gaussian model."}
   ...
   set_bold(1, everywhere) %>% 
   style_headers(bold = TRUE, text_color = "red") %>% 
   set_caption('Model choice criteria values for the spatio-temporal Gaussian model.')
```
