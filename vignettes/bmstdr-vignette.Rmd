...
The current package `r Rpackage("bmstdr")` sets out to help  researchers in applied sciences model a large variety of spatial and spatio-temporal data using a multiplicity of packages but by using only three commands with different options. Point reference spatial data, where each observation comes with a single geo-coded location reference such as a latitude-longitude pair, can be analyzed by fitting several spatial   and spatio-temporal models using `R` software packages such as
...
A particular package is chosen with the `package=` option to the `r Rpackage("bmstdr")` model fitting  routines `Bspatial` for point reference spatial only data and
`Bsptime` for spatio-temporal data. In each of these cases a Bayesian linear model, which can be fitted with the option `package="none"` provides a base line for model comparison purposes. For areal unit data modeling the `r Rpackage("bmstdr")` function `Bcartime` provides opportunities for model fitting using three packages: 
...
For areal unit data modeling the `r Rpackage("bmstdr")` function `Bcartime` provides opportunities for selected model fitting using `r CRANpkg("CARBayes")` [@LeeCARBayes2021], and  `r CRANpkg("CARBayesST")` [@CarBayesST] 
...
```{r nysites, fig.cap="28 air pollution monitoring sites in New York", echo=FALSE, eval=TRUE, fig.width=5, fig.height=4}
...
The `r Rpackage("bmstdr")` package includes the function `Bspatial` for fitting regression models to point referenced spatial data.
...
R packages for model fitting. The exact form of the models in each case is documented in Chapter 6 of the book @Sahubook. 
 ...
 The return value of `Bspatial` is a list of class `r Rpackage("bmstdr")`  providing  parameter estimates, and if requested model choice statistics and validation predictions
 and statistics. The S3methods `print, plot, summary, fitted`, and `residuals` have been
 implemented for objects of the `r Rpackage("bmstdr")` class.  Thus the user can give the commands 
 such as `summary(M1)` and `plot(M1)` where `M1` is the model fitted object .  
 

## Fitting independent error regression models

The `r Rpackage("bmstdr")` package allows us to fit the base linear regression model given by: 
\begin{equation}
...
provided in @Sahubook. Here  M0 is the intercept only model for which the results are obtained using the  following `r Rpackage("bmstdr")` command,
```{r, echo=TRUE, eval=FALSE}
...
The `r Rpackage("bmstdr")` command for performing validation needs an additional argument `validrows` which are the row numbers of the supplied data frame which should be used for validation. Thus the commands for validating at the
...
Table \@ref(tab:mvalidnyspatial) presents the validation statistics for all five models.  Coverage is 100% for all five models and the validation performances are comparable. Model  `M2` with $\phi=0.4$ can  be used as the best model if it is imperative that one must be  chosen.  
...
```{r valplot1, echo=T, eval=T, message=FALSE, results='hide', fig.cap="Observations and validation predictions with the 95% intervals."}
...
$\epsilon({\bf s}_i, t)$ give rise to different models. Variations of these models have been described in @Sahubook.  The `r Rpackage("bmstdr")` function `Bsptime` has been developed to fit these models. 
...
The fitted model objects `M1` and `M2` are of class `r Rpackage("bmstdr")` and these can be explored using the S3 methods `print, plot`, `summary` and `residuals`. 
...
```{r valplot21, echo=TRUE, eval=TRUE, message=FALSE, results='hide', fig.cap="Observations and predictions along with the 95% intervals."}
...
several plots depending on the contents of the model fitted object. For example, this always draws a residuals against fitted values plot. The `plot` command for objects of class `r Rpackage("bmstdr")` can take  additional arguments such as `segments = F` 
...
The `spTimer` package @spTimer can be used to fit, predict and forecast using various spatio-temporal models. This package also offers a great deal of flexibility in data modeling since it can model segmented time series data and also mixture of discrete and continuous data such as precipitation. The `r Rpackage("bmstdr")` function `Bsptime` can implement these models by invoking the  `package="spTimer"` option. 
For example:
...
```{r vsites3, echo=FALSE, eval=TRUE, fig.cap="25 fitted sites and 3 validation sites (numbered) in  New York", fig.width=7, fig.height=4.5}
nymap <- map_data(database="state",regions="new york")
s <- c(1, 5, 10)
...
The `r Rpackage("bmstdr")` package includes the data set `gridnysptime` which contains the prediction data for 100 locations within the state of New York. Here is the 
...
```{r residM5, echo=TRUE, eval=TRUE, results='hide', fig.cap="Multiple time series plot of residuals for model M5" }
a <- residuals(M5)
```
...
```
The AR models `M5` and `M6` produces the following results in Table \@ref(tab:mcvm5m6). 
```{r, echo=FALSE, eval=TRUE}
   table5 %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(2)     %>%
...
```
Parameter estimates from the two models are shown in Table \@ref(tab:mparm-m5-m6). The `r Rpackage("INLA")` implemented model autoregressive nodel `M6` has been fitted without an intercept
as this has been seen to be better than `r CRANpkg("spTimer")` model `M5`. Other parameter estimates are somewhat comparable but there are large differences  also, see e.g., the estimates of the autoregressive parameter $\rho$ and the spatial decay parameter $\phi$. These differences are expected as the two packages use different model parameterisations and prior distributions. The current package `r Rpackage("bmstdr")`  facilitates this sort of model comparison without much additional programming effort.  
```{r, echo=FALSE, eval=TRUE}
   table6 %>%
   as_hux(add_colnames = FALSE) %>%
   set_number_format(3)     %>%
...
```{r speffects,  echo=TRUE, eval=TRUE, message=FALSE, tidy=TRUE, fig.cap="Spatial effects of maximum temperature from model M7"}
out <- M7$fit
dim(out$betasp)
a <- out$betasp
u <- c(t(out$betasp))
...
```{r temporaleffects, echo=TRUE, eval=TRUE, message=FALSE, tidy=TRUE, fig.cap="Temporal effects  of wind spped from model M7"}
b <- out$betatp
tn <- nrow(b)
itmax <- ncol(b)
tids <- 1:tn 
...
```{r spbplot34, echo=FALSE, eval=TRUE, fig.cap="Regression parameter estimates from the dynamic model fitted by spBayes"}
library(ggpubr)
...
number of locations called the knots. These knots can be specified by a `knots.coords` argument in `spTimer`. In `r Rpackage("bmstdr")` the knots may also be specified by a `g_size` argument which may define a square or a rectangular equi-spaced grid covering the 
range of coordinates of the data locations. 

The `r Rpackage("bmstdr")` command to fit a  model with a square $5 \times 5$ grid of knot locations is given by:    
```{r, echo=TRUE, eval=TRUE, message=FALSE, results='hide'}
...
The `r Rpackage("bmstdr")` package function can be used compare all the model fits and the performance of the models as evaluated by the four validation statistics. 
...
WAIC and the DIC since those are not available in the `r Rpackage("bmstdr")` package for all the models. @Sahubook provides further discussion of the results. 
...
be another model, e.g. auto-regressive,  implemented using `stan`,  that may perform better than the `spTimer` models.  The worth of this illustration lies in the comparison itself. Using the `r Rpackage("bmstdr")` package it is straightforward to compare different models implemented in different packages without having to learn and program the individual packages. 
...
deep ocean is `deep`, we use the 
 command 
```{r deepmodfitting, echo=T, eval=TRUE, results='hide', message=FALSE}
...
The model output is not shown here. But we obtain an  annual prediction map by averaging over the model equation  \@ref(eq:marginalytocean) at each data location. The posterior samples contained in `M2` are used to sample the annual predictions and then those samples are linearly interpolated using the `r CRANpkg("akima")` library to obtain the  prediction map in Figure \@ref(fig:oceanplot) and the uncertainty map in \@ref(fig:oceanplotsd). 
```{r processdeep, echo=FALSE, eval=TRUE, message=FALSE}
library(rstan)
...
```{r oceanplot, message=FALSE, results='hide',  echo=FALSE, eval=TRUE, fig.cap="Predicted annual temperature  map of the  deep ocean in 2003"}
patldeep
```

```{r oceanplotsd, message=FALSE, results='hide',  echo=FALSE, eval=TRUE, fig.cap="Map of standard deviations of the predicted temperature in the   deep ocean in 2003"}
 patlsd
```
...
The `engtotals` data set  presents the number of deaths due to Covid-19 during the  peak from March 13 to July 31, 2020 in the 313 Local Authority Districts, Counties  and Unitary Authorities  in England.

```{r covidrate, echo=T, eval=T, fig.cap="Covid death rate per 100,000 during Mar 13 to Jul 31, 2020"}
...
<!-- caption = "Model choice statistics for the normal regression model fitted to the engtotals data set") --> 
...
The validation statistics reported in Table \@ref(tab:m2st-valid) show that the AR model is indeed slightly worse than the chosen Anova model.  
...
```{r obsvpredplot-m2st, echo=TRUE, eval=TRUE, message=FALSE, results='hide',  fig.show='hide'}
Ncar <- 5000
burn.in.car <- 1000
thin <- 10
...
M2stinla.0  <- Bcartime(data=engdeaths, formula=f2inla, W=Weng, scol =scol, tcol=tcol,  
                        offsetcol="logEdeaths",  model=model,  link="log", family="poisson", package="inla", validrow=vs, N=inlaN, burn.in=0) 
...
ar2valid <- b$pwithseg
```
```{r obsvpredplot, echo=FALSE, eval=TRUE, fig.width=12, fig.cap="Predictions with 95% limits against observations for two models: AR (2) on the left panel and INLA on the right panel."}
library(ggpubr)
